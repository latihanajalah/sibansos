...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
    ...
    <a href="{{ route('verifikasi.index') }}" class="btn btn-outline-secondary d-flex align-items-center gap-2">
        <i class="bi bi-arrow-left"></i> Kembali ke Daftar
    </a>
</div>

<x-breadcrumb :items="['Verifikasi' => route('verifikasi.index'), 'Detail Verifikasi' => '#']" />
...
<div class="row g-4">
    ...
    <div class="col-lg-8">
        ...
        @else
            <div class="alert alert-warning border-0 p-4 rounded-4">
                <h6 class="fw-bold"><i class="bi bi-exclamation-triangle-fill me-2"></i> Survei Belum Diisi</h6>
                Petugas lapangan belum melakukan survei atau mengunggah data survei untuk pengajuan ini.
            </div>
        @endif
        ...
        @if($pengajuan->status === 'menunggu_verifikasi')
            <div class="card card-saas border-0 p-4">
                <h5 class="fw-bold text-dark border-bottom pb-2 mb-4">
                    <i class="bi bi-shield-check text-primary me-2"></i> Form Keputusan Verifikasi
                </h5>

                <form action="{{ route('verifikasi.verify', $pengajuan) }}" method="POST" id="form-verifikasi">
                    @csrf

                    <div class="mb-4">
                        <label class="form-label fw-bold d-block mb-3">Keputusan Admin <span class="text-danger">*</span></label>
                        <div class="row g-3">
                            <div class="col-md-4">
                                <div class="card border p-3 h-100 btn-select-card bg-light" id="card-setujui" style="cursor: pointer;" onclick="selectDecision('setujui')">
                                    ...
                                    <small class="text-muted d-block mt-2">Teruskan pengajuan ke pimpinan untuk persetujuan akhir.</small>
                                </div>
                            </div>

                            <div class="col-md-4">
                                <div class="card border p-3 h-100 btn-select-card bg-light" id="card-revisi" style="cursor: pointer;" onclick="selectDecision('revisi')">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="keputusan" id="keputusan-revisi" value="revisi" required>
                                        <label class="form-check-label fw-bold text-warning" for="keputusan-revisi">
                                            <i class="bi bi-arrow-repeat me-1"></i> Revisi
                                        </label>
                                    </div>
                                    <small class="text-muted d-block mt-2">Kembalikan ke petugas untuk merevisi data survei.</small>
                                </div>
                            </div>

                            <div class="col-md-4">
                                <div class="card border p-3 h-100 btn-select-card bg-light" id="card-tolak" style="cursor: pointer;" onclick="selectDecision('tolak')">
                                    <div class="form-check">
                                    ...
                                    <small class="text-muted d-block mt-2">Tolak pengajuan bantuan sosial ini secara permanen.</small>
                                </div>
                            </div>
                        </div>
                        @error('kepecision')
                    ...
                    <div class="mb-4">
                        <label for="catatan" class="form-label fw-bold" id="catatan-label">Catatan Verifikasi</label>
                        <textarea name="catatan" id="catatan" class="form-control @error('catatan') is-invalid @enderror" rows="4" placeholder="Tuliskan catatan verifikasi, alasan penolakan, atau instruksi revisi..."></textarea>
                        <div class="invalid-feedback" id="catatan-feedback">Catatan wajib diisi jika Anda memilih keputusan Revisi atau Tolak.</div>
                        @error('catatan')
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        <a href="{{ route('verifikasi.index') }}" class="btn btn-outline-secondary px-4">Batal</a>
        ...
        @else
            <div class="alert alert-light border border-light p-4 rounded-4 d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <span class="text-muted small">Status pengajuan saat ini adalah <strong>{{ str_replace('_', ' ', $pengajuan->status) }}</strong>. Form verifikasi hanya dapat diakses saat pengajuan berstatus <strong>menunggu_verifikasi</strong>.</span>
                </div>
            </div>
        @endif
    </div>
...
<script>
    function selectDecision(decision) {
        ...
        document.querySelectorAll('.btn-select-card').forEach(function (card) {
            card.classList.remove('border-success', 'border-warning', 'border-danger', 'bg-white');
        ...
        if (decision === 'setujui') {
            ...
            document.getElementById('catatan-label').innerHTML = 'Catatan Verifikasi <span class="fw-normal text-muted">(Opsional)</span>';
            document.getElementById('catatan').required = false;
        } else if (decision === 'revisi') {
            card.classList.add('border-warning');
            document.getElementById('catatan-label').innerHTML = 'Catatan Verifikasi <span class="text-danger">*</span>';
    ...
    document.getElementById('form-verifikasi')?.addEventListener('submit', function (e) {
        ...
        if ((selectedValue === 'revisi' || selectedValue === 'tolak') && catatan === '') {
            e.preventDefault();
